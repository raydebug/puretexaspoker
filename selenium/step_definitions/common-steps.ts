...
Then('the page should not contain {string}', async function(text: string) {
  const helpers: WebDriverHelpers = this.helpers
...
Then('element {string} should be visible', async function(selector: string) {
  const helpers: WebDriverHelpers = this.helpers
  await helpers.shouldBeVisible(selector)
  console.log(`✅ Element is visible: ${selector}`)
})

Then('element {string} should not be visible', async function(selector: string) {
  const helpers: WebDriverHelpers = this.helpers
  await helpers.shouldNotBeVisible(selector)
  console.log(`✅ Element is not visible: ${selector}`)
})

Then('element {string} should exist', async function(selector: string) {
  const helpers: WebDriverHelpers = this.helpers
  await helpers.shouldExist(selector)
  console.log(`✅ Element exists: ${selector}`)
})

Then('element {string} should not exist', async function(selector: string) {
  const helpers: WebDriverHelpers = this.helpers
