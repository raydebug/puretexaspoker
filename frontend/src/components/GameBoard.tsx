...
const WinnerPopup = styled.div<{ show: boolean }>`
  position: fixed;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
  ...
  animation: ${props => props.show ? 'popupBounce 0.5s ease-out' : 'none'};
...
export const GameBoard: React.FC<GameBoardProps> = ({
  ...
  useEffect(() => {
    ...
    if (prevState.phase === 'showdown' && currentState.phase === 'waiting') {
      ...
      const winners = currentState.players.filter(player => {
  ...
  return (
    <GameContainer data-testid="game-table">
      ...
      <WinnerPopup show={showWinnerPopup} data-testid="winner-popup">
        ...
        <div>
          <span className="winner-name">{winnerInfo?.name}</span> wins the hand!
        </div>
